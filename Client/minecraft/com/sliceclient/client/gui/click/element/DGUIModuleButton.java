...
public class DGUIModuleButton implements Util {
	...
	public void render(int mx, int my, ScaledResolution sr, float x, float y, int index) {
		if (ModGUI.instance().selMod == this.module) {
			if (ModGUI.drawRect(new Rect(x - 3, y + 4, x + 10, y + 18), new Color(0x4e32a8).getRGB())
					...
					if (option instanceof DoubleProperty) {
						...
						if (percentBar >= -9990.0) {
							dragging = ModGUI
									...
									.isMouseHovering(mx, my) && Mouse.isButtonDown(0);
							ModGUI.drawRect(new Rect(x, this.index + 12.0, x + percentBar * width, this.index + 14.5),
									-1);
							ModGUI.drawRect(new Rect(x + percentBar * width - 1.0, this.index + 12.0,
						...
						if (ModGUI
								.drawRect(new Rect(x, this.index + 8,
										...
										ModGUI.BACKGROUND_COLOR.brighter().brighter().getRGB())
								.isMouseHovering(mx, my) && Mouse.isButtonDown(0)) {
						...
						if (ModGUI.drawRect(new Rect(x - 2, y + this.index - 12, x + 10, y + this.index - 2),
								ModGUI.BACKGROUND_COLOR.darker().getRGB()).isMouseHovering(mx, my)) {
						...
						if (stringOption.getValue()) {
							ModGUI.drawRect(new Rect(x - 2, y + this.index - 12, x + 10, y + this.index - 2), -1);
						}
					}
					this.index += 20;
				}
			if (ModGUI
					.drawRect(new Rect(x, this.index + 12, x + 40, this.index + 26),
							ModGUI.BACKGROUND_COLOR.brighter().getRGB())
					.isMouseHovering(mx, my) && Mouse.isButtonDown(0)) {
				listening = true;
			}
		...
		this.index = (int) y + 21;
		if (ModGUI.instance().selMod == null) {
			this.rects.add(ModGUI.drawRect(new Rect(x - 3, y + index - 11, x + 100, y + index + 11),
					ModGUI.BACKGROUND_COLOR.brighter().getRGB()));
			for (Rect rect : this.rects) {
				if (rect.isMouseHovering(mx, my) && Mouse.isButtonDown(1)) {
					ModGUI.instance().selMod = this.module;
					Mouse.destroy();
					try {
						Mouse.create();
					} catch (LWJGLException e) {
			...
			this.rects.add(ModGUI.drawRect(new Rect(x + 70, y + index - 5, x + 98, y + index + 5),
					ModGUI.BACKGROUND_COLOR.getRGB()));
			if (module.isToggled()) {
				this.rects.add(ModGUI.drawRect(new Rect(x + 92, y + index - 3, x + 96, y + index + 3),
						!module.isToggled() ? Color.red.darker().getRGB() : Color.green.darker().getRGB()));
			} else {
				this.rects.add(ModGUI.drawRect(new Rect(x + 72, y + index - 3, x + 76, y + index + 3),
						!module.isToggled() ? Color.red.darker().getRGB() : Color.green.darker().getRGB()));
			}
			for (Rect rect : this.rects) {
